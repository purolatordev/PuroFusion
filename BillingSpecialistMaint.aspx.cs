...
public partial class BillingSpecialistMaint : System.Web.UI.Page
{
    ...
    protected void loadUsers()
    {
        ...
        List<clsBillingSpecialist> qShipMeth = SrvBillingSpecialist.GetBillingSpecialistView();
    ...
    protected void rgITBA_InsertCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            clsBillingSpecialist oRow = populateObj(userControl);
            string insertMsg = "";
            if (IsValid)
            {
                if (oRow != null)
                {
                    insertMsg = SrvBillingSpecialist.InsertBillingSpecialist(oRow);
                    if (insertMsg == "")
                    {
    ...
    protected void rgITBA_UpdateCommand(object source, GridCommandEventArgs e)
    {
        try
        {
            ...
            Label errorMsg = (Label)userControl.FindControl("lblErrorMessage");
            clsBillingSpecialist oRow = populateObj(userControl);
            oRow.idBillingSpecialist = Convert.ToInt16((userControl.FindControl("hdnITBAID") as HiddenField).Value);
            string updateMsg = "";
            if (IsValid)
            {
                if (oRow != null)
                {
                    updateMsg = SrvBillingSpecialist.UpdateBillingSpecialist(oRow);
                    if (updateMsg == "")
                    {
    ...
    private clsBillingSpecialist populateObj(UserControl userControl)
    {
        clsBillingSpecialist oRow = new clsBillingSpecialist();
